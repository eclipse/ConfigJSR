...
A jakartaconfig implementation must provide <<ConfigSource,ConfigSources>> for the following data out of the box:
...
* A `ConfigSource` for each property file `META-INF/jakartaconfig.properties` found on the classpath. (default ordinal = 100).
...
To add a custom `ConfigSource`, implement the interface `jakarta.config.spi.ConfigSource`.
...
Then register your implementation in a resource file `/META-INF/services/jakarta.config.spi.ConfigSource` by including the fully-qualified class name of the custom implementation in the file.
...
A custom `ConfigSourceProvider` must implement the interface `jakarta.config.spi.ConfigSourceProvider`.
Register your implementation in a resource file `/META-INF/services/jakarta.config.spi.ConfigSourceProvider` by including the fully-qualified class name of the custom implementation/s in the file.
...
public class ExampleYamlConfigSourceProvider
        implements jakarta.config.spi.ConfigSourceProvider {
    @Override
